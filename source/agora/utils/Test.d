...
unittest
{
    ...
    auto txes = makeChainedTransactions([WK.Keys.A.address], genesisSpendable(), 1);
...
unittest
{
    ...
    auto gen_key = getGenesisKeyPair();
    const(Transaction)[] txes = makeChainedTransactions([WK.Keys.A.address], genesisSpendable(), 1);
    txes = makeChainedTransactions([WK.Keys.A.address], txes, 1);
}
...
unittest
{
    ...
    auto txes = makeChainedTransactions([key_pair.address], [GenTx], 1, Amount.MinFreezeAmount);
    foreach (idx; 0 .. Block.TxsInBlock)
    {
        assert(txes[idx].inputs.length == 1);
        assert(txes[idx].inputs[0].index == idx);
    ...
    ***************************************************************************/
    ...
    public Transaction sign (TxType type = TxType.Payment) @safe
    {
        ...
        assert(this.data.outputs.length || this.leftover.value > Amount(0),
               "Onput-less transactions are not valid");
        ...
        {
            auto ownerKP = this.getOwnerKeyPair(in_.output.address);
    ...
    /***************************************************************************
        ...
        Finds a well-known `KeyPair` corresponding to the `PublicKey`.

        Params:
            address = finding `PublicKey`

        Returns:
            returns KeyPair.init if `address` is not found.

    ***************************************************************************/

    private KeyPair getOwnerKeyPair (scope const PublicKey address) @safe
    {
        // FIXME: Using the binary version didn't work (?)
        if (address.toString() == "GCOQEOHAUFYUAC6G22FJ3GZRNLGVCCLESEJ2AXBIJ5BJNUVTAERPLRIJ")
            return  () @trusted {
                return KeyPair.fromSeed(Seed.fromString(
                    "SCT4KKJNYLTQO4TVDPVJQZEONTVVW66YLRWAINWI3FZDY7U4JS4JJEI4"));
            }();

        return WK.Keys[address];
    }
...
/// Returns: A range ofTransactions which are spendable in the Genesis block
